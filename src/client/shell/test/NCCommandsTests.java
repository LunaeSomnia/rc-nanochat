...
class NCCommandsTests extends NCCommands {
    ...
    void testStringToCommand() {
        ...
        assertTrue(IntStream.range(0, _valid_user_commands_str.length).allMatch(i -> NCCommands
                .stringToCommand(NCCommands._valid_user_commands_str[i]) == NCCommands._valid_user_commands[i]));
    }

    @Test
    void testStringToCommandCaseRegardless() {
        // Test strintToCommand works for all commands, regardless of the case
        assertTrue(IntStream.range(0, _valid_user_commands_str.length).allMatch(i -> NCCommands.stringToCommand(
                NCCommands._valid_user_commands_str[i].toUpperCase()) == NCCommands._valid_user_commands[i]));
    }
